...
const AdminTransactions = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const { data } = await api.get("/admin/transactions", { params: { search, type, page, limit: 10 } });
  ...
  return (
    <DashboardLayout>
      <h1 className="text-2xl font-bold text-navy-900">All Transactions</h1>
      <p className="mt-1 text-sm text-navy-700/60">Every deposit, withdrawal, and transfer across the bank</p>
      ...
      <div className="card mt-6">
        ...
        {loading ? (
          ...
          <EmptyState icon={<MdReceiptLong />} title="No transactions found" />
        ) : (
          <>
              ...
              <table className="w-full text-left text-sm">
                <thead>
                  <tr className="border-b border-navy-700/10 text-navy-700/50">
                    ...
                    <th className="whitespace-nowrap py-2.5 pr-4 font-medium">Customer</th>
                    ...
                    <th className="whitespace-nowrap py-2.5 text-right font-medium">Amount</th>
                  </tr>
                </thead>
                  ...
                  {transactions.map((tx) => {
                    ...
                    return (
                      <tr key={tx._id}>
                        ...
                        <td className="whitespace-nowrap py-3 pr-4 font-medium text-navy-900">{tx.user?.name || "—"}</td>
                        ...
                        <td className={`whitespace-nowrap py-3 text-right font-semibold ${meta.sign === "+" ? "text-emerald-dark" : "text-rose-600"}`}>
            ...
            <div className="mt-5 flex items-center justify-between text-sm">
              <p className="text-navy-700/50">Page {pagination.page} of {pagination.pages} · {pagination.total} total</p>
              <div className="flex gap-2">
                <button className="btn-secondary !px-3 !py-1.5" disabled={page <= 1} onClick={() => setPage((p) => p - 1)}>Previous</button>
                <button className="btn-secondary !px-3 !py-1.5" disabled={page >= pagination.pages} onClick={() => setPage((p) => p + 1)}>Next</button>
              </div>
            </div>
          </>
        )}
